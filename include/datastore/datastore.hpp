...
class Datastore {
 public:
  virtual ErrCode Open() = 0;   //! Open storage
  virtual ErrCode Close() = 0;  //! Close storage
  virtual ErrCode Put(
      const Key& key,
      const Value& value) = 0;            //! Put pair{key, value} in storage
  virtual Value Get(const Key& key) = 0;  //! Get value by key in storage
  virtual ErrCode Delete(const Key& key) = 0;  //! Delete value by key in storage
