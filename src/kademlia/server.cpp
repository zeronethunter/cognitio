...
Server::Server(Identifier id)
    : __id(id), __routingTable(id), __alive(false), __repository(id) {}
...
void Server::start(std::string ip, uint32_t port) {
  ...
  __services.push_back(std::shared_ptr<CommunicationServiceImp>(
      new CommunicationServiceImp(this)));

  __builder.reset(new grpc::ServerBuilder);
  __builder->AddListeningPort(ip + ":" + std::to_string(port),
  ...
  __serverThread.reset(new std::thread([this]() { __server->Wait(); }));
      ...
      do {
        ...
        if (!isAlive()) {
          ...
          std::chrono::duration_cast<std::chrono::milliseconds>(end - begin)
              .count() < Kademlia::PROCESS_MILLISECONDS_INTERVAL);
    }
  }));
}
...
Identifier Server::id() const { return __id; }
...
FindNodeAnswer Server::findNode(const ConnectionInfo& caller,
                                const ConnectionInfo& node,
  ...
  while (!closestPeers.found() && prevClosestPeers != closestPeers) {
    ...
    for (auto peer : closestPeers) {
      std::shared_ptr<grpc::Channel> channel =
          grpc::CreateChannel(peer.ip() + ":" + std::to_string(peer.port()),
                              grpc::InsecureChannelCredentials());

      CommunicationClientImp communication(channel, this);

      kademlia_rpc::FindNodeQuery findQuery;
      kademlia_rpc::ConnectionInfo* caller_alloc =
          new kademlia_rpc::ConnectionInfo();
...
bool Server::store(std::string key, std::string value) {
  ...
  Identifier keyHash = Utils::hash_key(key, Kademlia::GUID_SPACE)
...
ConnectionInfo Server::address() const { return __address; }
...
RoutingTable& Server::routingTable() { return __routingTable; }

bool Server::isAlive() const { return __alive; }
